...
result={"bc":bc,"step":stepname,"type":bt,"region":${region}}`,
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Define a time-varying amplitude that scales loads/BCs over time. amplitudeType: TABULAR (data=[[time,value],...]), SMOOTH_STEP (data=[[time,value],...]), DECAY (data=[initialValue, decayTime]), or PERIODIC (data={startTime,frequency,amplitude,cycle,phase}). timeSpan: STEP (default) or TOTAL. Use for ramp/sinusoidal/pulse loading on a load or BC.',
      parameters: {
        model: { type: 'string', required: true, description: 'Model name' },
        ...
        data: {
          ...
          required: true,
          description:
            'JSON: for TABULAR/SMOOTH_STEP "[[0,0],[1,1]]"; for DECAY "[initialValue, decayTime]"; for PERIODIC "{\\"startTime\\":0,\\"frequency\\":0.5,\\"amplitude\\":5,\\"cycle\\":1,\\"phase\\":0}"',
        },
      },
      output: {
        schema: { type: 'object', additionalProperties: true },
      ...
      async execute(args, exec) {
        ...
        if (!['STEP', 'TOTAL'].includes(tsp)) throw new Error('timeSpan must be STEP|TOTAL')
        let data: unknown
        try {
...
result={"amplitude":a.name,"type":atype,"points":pts,"timeSpan":("STEP" if tsp==STEP else "TOTAL")}`,
          config.timeoutMs,
          exec.signal,
        )
        return r.value as JsonRecord
  ...
  ctx.tools.register(
    defineTool({
      ...
      async execute(args, exec) {
        ...
        if (!['TEMPERATURE', 'STRESS', 'VELOCITY'].includes(ftype)) throw new Error('fieldType must be TEMPERATURE|STRESS|VELOCITY')
        const region = JSON.stringify(String(args.region))
        const inst = args.instance ? JSON.stringify(String(args.instance)) : 'None'
        const step = args.step ? JSON.stringify(String(args.step)) : JSON.stringify('Initial')
...
result={"field":f.name,"type":ft,"step":stepname,"region":${region}}`,
          config.timeoutMs,
          exec.signal,
        )
        return r.value as JsonRecord
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Configure result output requests. outputType: FIELD (full-field contour data) or HISTORY (time series at a region). variables: array of output variables, e.g. ["S","U","RF"] (FIELD) or ["U","RF"] (HISTORY). Use frequency=1 (every increment), numberOfIntervals=N (fixed frames), or lastIncrement=true (final only). region+instance only for HISTORY (a node/region set).',
      parameters: {
        model: { type: 'string', required: true, description: 'Model name' },
      ...
      async execute(args, exec) {
        ...
        if (!['FIELD', 'HISTORY'].includes(otype)) throw new Error('outputType must be FIELD|HISTORY')
        let vars: unknown
        try {
...
result={"output":o.name,"type":otype,"step":stepname,"vars":list(vars)}`,
          config.timeoutMs,
          exec.signal,
        )
        return r.value as JsonRecord
