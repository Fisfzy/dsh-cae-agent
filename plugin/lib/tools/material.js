...
export function registerMaterial(ctx, config) {
    const br = { host: config.host, port: config.port };
    ctx.tools.register(defineTool({
        name: 'abaqus_create_material',
        description: 'Create an Abaqus material and place it on the given model. `props` is a JSON object mapping property names to values, e.g. ' +
...
result={"model":${model},"name":mat.name,"materialExists":mat.name in m.materials,"properties":${JSON.stringify(props)}}`, config.timeoutMs, exec.signal);
            return r.value;
        },
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Create a section referencing an existing material and assign it to a region of a part. Region is chosen by a named set on the part/assembly, or by bare geometric cell/face/edge indices (0-based). sectionType: solid|shell|beam. thickness only for shell.',
        parameters: {
        ...
        async execute(args, exec) {
            ...
            if (!VALID_SECTION_TYPES.includes(stype))
                throw new Error(`sectionType must be ${VALID_SECTION_TYPES.join('|')}`);
...
result={"model":${model},"part":${part},"section":secname,"material":${mat},"type":stype,"assignedRegion":reg.name}`, config.timeoutMs, exec.signal);
            return r.value;
        },
        timeoutMs: config.timeoutMs,
        isConcurrencySafe: () => false,
