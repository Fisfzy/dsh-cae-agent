...
export function registerMesh(ctx, config) {
    const br = { host: config.host, port: config.port };
    ctx.tools.register(defineTool({
        name: 'abaqus_generate_mesh',
        description: 'Seed and generate a mesh on a part in a model. elementFamily: solid (default) or shell. solid -> C3D8R (hex) / C3D4R (tet) via elemShape auto; shell -> S4R. size: approximate global seed size; if omitted, auto = bounding-box diagonal / 10.',
        parameters: {
        ...
        async execute(args, exec) {
            ...
            if (!['solid', 'shell'].includes(family))
                throw new Error('elementFamily must be solid|shell');
            const elemShape = String(args.elemShape || 'hex');
            if (!['hex', 'tet'].includes(elemShape))
                throw new Error('elemShape must be hex|tet');
            const r = await runKernelCode(br, `import math
...
p.generateMesh()
result={"part":part,"family":family,"size":size,"elements":len(p.elements),"nodes":len(p.nodes),"cells":len(p.cells) if hasattr(p,"cells") else None}`, config.timeoutMs, exec.signal);
            return r.value;
        },
