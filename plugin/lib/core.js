 ...
 */
export function bridgeRequest(handle, method, params, timeoutMs = DEFAULT_TIMEOUT_MS, signal) {
    return new Promise((resolve, reject) => {
        ...
        socket.on('error', (err) => {
            ...
            finish(reject, new Error(`Cannot reach Abaqus socket bridge at ${handle.host}:${handle.port}. ` +
 ...
 */
export async function runKernelCode(handle, code, timeoutMs = DEFAULT_TIMEOUT_MS, signal) {
    ...
    if (!result.ok) {
        ...
        if (result.recovery) {
            ...
            if (r.parent_object_path)
                parts.push(`  Object: ${r.parent_object_path}`);
            if (r.possible_keys)
                parts.push(`  Similar keys: ${safeStringify(r.possible_keys)}`);
            if (r.callable_signature)
                parts.push(`  Signature: ${r.callable_signature}`);
        }
        if (result.code_excerpt)
            parts.push(`  Code:\n${String(result.code_excerpt)}`);
        if (result.traceback_tail)
